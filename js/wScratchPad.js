...
(function($) {
  $.fn.wScratchPad = function(option, settings) {
    if (typeof option === 'object') {
      ...
      var elements = this.each(function() {
        ...
        if (data) {
          if (option === 'reset') { data.reset(); } else if (option === 'clear') { data.clear(); } else if (option === 'enabled') { data.enabled = settings === true; } else if ($.fn.wScratchPad.defaultSettings[option] !== undefined) {
            if (settings !== undefined) { data.settings[option] = settings; } else { values.push(data.settings[option]); }
          }
        }
      });
  ...
  ScratchPad.prototype = {
    generate: function() {
      ...
      this.sp =
        $('<div></div>')
        .css({ position: 'relative', backgroundSize: '100% 100%' })
        .append(
          $(this.canvas)
    ...
    reset: function() {
      ...
      var that = this;
      var _parsent = $(this.sp).parent();
      _parsent.find(this.sp).remove();
      this.timer = setTimeout(function() {
        _parsent.append(that.generate());
    ...
    scratchFunc: function(e, $this, event) {
      var offset = {};
      if ($ === window.Zepto) {
        offset.pageX = e.layerX;
        offset.pageY = e.layerY;
      }
      if ($ === window.jQuery) {
        offset.pageX = Math.floor(e.pageX - $this.canvas_offset.left);
        offset.pageY = Math.floor(e.pageY - $this.canvas_offset.top);
      }

      $this['_scratch' + event](offset, $this);

      if ($this.settings['scratch' + event]) $this.settings['scratch' + event].apply($this, [offset, $this.scratchPercentage($this)]);
    },
    ...
    _scratchMove: function(e, $this) {
      $this.ctx.lineTo(e.pageX, e.pageY);
      $this.ctx.stroke();
    },

    _scratchUp: function(e, $this) {
      $this.ctx.closePath();
    },
  }
